...
class AIService:
    ...
    async def summarize_with_style(self, text: str, style: str = 'generic') -> dict:
        ...
        response = self.openai_client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": prompt_config['user_template'].format(text=text)}
            ],
            max_tokens = self._get_max_tokens_for_style(style)
        )
        
        usage = response.usage 
        cost = self.calculate_cost(response.model, usage.prompt_tokens, usage.completion_tokens)
        return {
    ...
    def calculate_cost(self, model: str, prompt_tokens: int, completion_tokens: int) -> float:
        ...
        total = (prompt_tokens / 1000) * rates["prompt"] + (completion_tokens / 1000) * rates["completion"]
        return total
